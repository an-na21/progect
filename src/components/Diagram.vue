<template>
  ...
  v-for ='task in tasks'
  :key="task.index"
  v-bind:dataDasharray="task"></ToDo>
...
export default {
    ...
    computed: {
        ...
        degRotate() {
            ...
            const percent = Number(((this.dataDasharray[0] * 100) / this.dataDasharray[1]).toFixed(1));
    ...
    watch: {
        dataDasharray: {
            handler() {
                ...
                const percent = ((this.dataDasharray[0] * 100) /
                    this.dataDasharray[1]).toFixed(1);
